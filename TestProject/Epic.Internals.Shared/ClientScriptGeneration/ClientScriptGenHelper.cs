...
namespace Epic.Internals.Shared.ClientScriptGeneration
{
	...
	internal class ClientScriptGenHelper
	{
		...
		public static string TryFindExtractedType(ClientScriptGenerator<ClientScriptObjectAttribute> clientGen, Type type)
		{
			string bestMatch = null;
			foreach (string oneNamespace in clientGen.NamespaceGroups.Keys)
			{
				foreach (TypeAttributePair<ClientScriptObjectAttribute> typeAttrPair in clientGen.NamespaceGroups[oneNamespace])
				{

...
namespace Epic.Internals.Shared.JavascriptObjects
{
	...
	public abstract class TypescriptGenerator<TAttributeType> : ClientScriptGenerator<TAttributeType> where TAttributeType : JavascriptExtractedAttribute
	{
		...
		protected override void Write()
		{
			foreach (string oneNamepace in NamespaceGroups.Keys)
			{
				...
				foreach (TypeAttributePair<TAttributeType> typeAttr in typesInNS)
				{
					...
					AddFormatLine(1, "export class {0} {{", TypeUtils.GetTypeName(typeAttr.TargetType));
					AddLine(1, "");
